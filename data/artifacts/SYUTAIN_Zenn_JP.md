...
**実数値（2026年4月3日時点、本番DB直接取得）：**
| 項目 | 値 |
|------|-----|
| Python | 51,689行 / 132ファイル |
| PostgreSQL | 45テーブル / 31,384イベント |
| 累計LLM呼び出し | 10,170回（74.0%ローカル） |
| 累計LLMコスト | ¥936.80 |
| ゴール処理 | 97件（完了52 / キャンセル35 / エスカレーション8） |
| タスク実行 | 1,066件 |
| 承認処理 | 364件（自動承認188 / 手動承認169 / 却下7） |
| SNS自動投稿 | 572件（Bluesky 298 / Threads 160 / X 112） |
| 情報収集 | 1,358件 |
| ペルソナ記憶 | 541件 |
...
[`tools/nats_client.py`](https://github.com/SYUTAIN-system/syutain_beta/blob/main/tools/nats_client.py)（247行）
...
[`episodic_memory`](https://github.com/SYUTAIN-system/syutain_beta/blob/main/tools/episodic_memory.py)はMemRL（Memory-based Reinforcement Learning）のアイデアを実装したもの。タスク実行のたびにエピソードを記録し、次回の類似タスクで有用だったエピソードのQ値を上げる。現在137エピソード、20スキルが自動抽出されている。
...
[`tools/loop_guard.py`](https://github.com/SYUTAIN-system/syutain_beta/blob/main/tools/loop_guard.py)（441行）
...
本番稼働の発動回数（3月19日〜4月3日、DB直接取得）:

```
層2（同一失敗クラスタ）:      27回 ← 最多
層7（Emergency Kill、予算）:   22回
層6（コスト＆時間ガード）:     3回
層8（セマンティックループ）:   1回
層3（プランナーリセット）:     1回
合計:                          54回
...
3月25日は4,491イベントが記録された最悪の日。セマンティックループ検知器がグローバルスコープで発動し、全ゴールが停止した。修正後、検知器をゴール単位に再設計。
...
[`agents/proposal_engine.py`](https://github.com/SYUTAIN-system/syutain_beta/blob/main/agents/proposal_engine.py)（978行）
...
[`tools/llm_router.py`](https://github.com/SYUTAIN-system/syutain_beta/blob/main/tools/llm_router.py)（1,108行）
...
| モデル | 呼び出し数 | コスト(円) |
|--------|-----------|-----------|
| qwen3.5-9b（BRAVO/CHARLIE） | 3,873 | ¥0 |
| nemotron-jp（日本語特化） | 2,489 | ¥0 |
| qwen3.5-4b（DELTA） | 1,157 | ¥0 |
| jina-embeddings-v3 | 1,136 | ¥11 |
| gemini-2.5-flash | 546 | ¥127 |
| deepseek-v3.2 | 304 | ¥24 |
| claude-haiku-4-5 | 224 | ¥226 |
| tavily-search | 140 | ¥280 |
| gpt-5.4 | 20 | ¥73 |
| **合計** | **10,170** | **¥936.80** |

ローカルモデル（qwen3.5 + nemotron）が全体の74.0%。API呼び出しは主に埋め込み生成（jina）、情報収集（tavily/jina-reader）、品質検証（gemini-flash/claude-haiku）に使われている。

### セマンティックキャッシュ

[`tools/semantic_cache.py`](https://github.com/SYUTAIN-system/syutain_beta/blob/main/tools/semantic_cache.py)（493行）
...
[`pdl/worker.sh`](https://github.com/SYUTAIN-system/syutain_beta/blob/main/pdl/worker.sh)（352行）
...
51,689行、45テーブル、10,170回のLLM呼び出し、54回のLoopGuard発動、572件のSNS自動投稿。
