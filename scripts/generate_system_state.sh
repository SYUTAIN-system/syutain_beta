...
- 絶対ルール: CLAUDE.md（22条）
- SSH: BRAVO=${REMOTE_SSH_USER}@${BRAVO_IP} / CHARLIE=${REMOTE_SSH_USER}@${CHARLIE_IP} / DELTA=${REMOTE_SSH_USER}@${DELTA_IP}
...
for NODE_INFO in "${BRAVO_IP:-127.0.0.1}:BRAVO:RTX5070-12GB" "${CHARLIE_IP:-127.0.0.1}:CHARLIE:RTX3080-10GB" "${DELTA_IP:-127.0.0.1}:DELTA:GTX980Ti-6GB"; do
  IFS=':' read -r IP NAME GPU <<< "$NODE_INFO"
  WORKER=$(ssh -o ConnectTimeout=3 ${SSH_USER}@$IP "systemctl is-active syutain-worker-$(echo $NAME | tr '[:upper:]' '[:lower:]')" 2>/dev/null || echo "?")
  NATS=$(ssh -o ConnectTimeout=3 ${SSH_USER}@$IP "systemctl is-active syutain-nats" 2>/dev/null || echo "?")
  OLLAMA=$(ssh -o ConnectTimeout=3 ${SSH_USER}@$IP "systemctl is-active ollama" 2>/dev/null || echo "?")
  MODEL=$(ssh -o ConnectTimeout=3 ${SSH_USER}@$IP "curl -s http://localhost:11434/api/tags 2>/dev/null | python3 -c \"import sys,json;d=json.load(sys.stdin);print(','.join(m['name'] for m in d.get('models',[])))\" 2>/dev/null" || echo "?")
