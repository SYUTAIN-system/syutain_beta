...
| BRAVO | 100.75.146.9 | Ubuntu | RTX 5070 12GB | Nemotron 9B JP + Qwen3.5-9B | LLM主力 + ブラウザ操作 |
| CHARLIE | 100.70.161.106 | Ubuntu (Win11デュアルブート) | RTX 3080 10GB | Nemotron 9B JP + Qwen3.5-9B | 副推論。Win11時は自動退避 |
| DELTA | 100.82.81.105 | Ubuntu | GTX 980Ti 6GB | Qwen3.5-4B | 監視 + 軽量タスク |
...
echo ""
echo "=== ノード疎通+LLM ==="
for NODE_INFO in "100.75.146.9:BRAVO" "100.70.161.106:CHARLIE" "100.82.81.105:DELTA"; do
  IFS=':' read -r IP NAME <<< "$NODE_INFO"
  STATUS=$(ssh -o ConnectTimeout=5 shimahara@$IP "systemctl is-active syutain-worker-* 2>/dev/null" 2>/dev/null | head -1)
  MODELS=$(ssh -o ConnectTimeout=5 shimahara@$IP "ollama list 2>/dev/null | tail -n +2 | awk '{print \$1}' | tr '\n' ',' " 2>/dev/null)
