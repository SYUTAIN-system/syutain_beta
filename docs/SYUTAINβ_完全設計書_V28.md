...
> **「これは、単なるAIエージェントではない。4台のPCがPhase 1初日から全て連携し、NATSとTailscaleで結ばれ、Web UIを通じてiPhoneからリアルタイム監視でき、18のコアエージェントと64のツールが自律的に認識・思考・行動・検証・停止判断を行い、目標だけを受けても道筋が変わっても自分で考え、自分で動き、自分で止まれる『自律分散型 事業OS』である。これは技術文書であり、同時にドキュメンタリーでもある。一人の人間とAIが、ゼロから収益を作り、進化し続ける過程の全記録。」**

**バージョン:** V28（2026年4月1日・完全設計書）
...
| 指標 | 値 |
|------|-----|
| Python | 48,339行 / 121ファイル |
| TypeScript | 6,765行 / 27ファイル |
| 総コード量 | 約55,100行 |
| PostgreSQL | 50+テーブル / 25,505イベント |
| ノード数 | 4台（ALPHA/BRAVO/CHARLIE/DELTA） |
| GPU | 4基（M4 Pro / RTX 5070 / RTX 3080 / GTX 980Ti） |
| 月間コスト | 約¥664（API実費） |
| ローカル推論率 | 85.1%（7,169件ローカル / 1,257件API） |
| 情報収集アイテム | 944件 |
| タスク実行数 | 926件 |
| ゴール数 | 86件 |
| 承認キュー処理 | 310件 |
...
- **85%ローカル推論**: Qwen3.5-9B/4Bによるコスト最小化
...
V28はV25（原典設計書）→V26→V27を経て到達した最新設計書である。V25はV20〜V24の全設計を再構成・統合した原典であり、過去設計は一切削除していない。V28は実稼働中のシステム状態を正として記述する。
...
| PostgreSQL 16 | ALPHA | 5432 | 共有状態DB（50+テーブル） |
...
## 10.5 実データ注入
content_pipelineがDB実績値を自動埋め込み: ローカル推論率85.1%、月間コスト¥664、稼働ノード数4台、タスク成功数926件、event_logから実際のエラー事例を引用。
...
## 15.1 persona_memory
PostgreSQL persona_memoryテーブル（522件）。priority_tier 1-4で管理。
...
## 17.2 MemRL
episodic_memory + Q-value。成功で上昇、失敗で停滞（下がらない）。学習対象: モデル選定精度、提案採用率、品質、ブラウザ層別成功率。
...
APScheduler（AsyncIOScheduler, timezone="Asia/Tokyo"）。約60ジョブ。
...
## 20.1 PostgreSQL（50+テーブル）

主要テーブル:

| テーブル | 件数 | 用途 |
|----------|------|------|
| tasks | 926 | タスク管理（15カラム） |
| goal_packets | 86 | ゴール管理（13カラム） |
| proposal_history | 58 | 提案履歴（13カラム） |
| approval_queue | 310 | 承認キュー（7カラム） |
| intel_items | 944 | 情報収集（10カラム） |
| llm_cost_log | 8,426 | LLMコスト（7カラム） |
| event_log | 25,505 | イベントログ（9カラム） |
| browser_action_log | 7,285 | ブラウザ操作（12カラム） |
| chat_messages | 224 | チャット（6カラム） |
| persona_memory | 522 | ペルソナ記憶（10カラム） |
| revenue_linkage | 0 | 収益紐付け（9カラム） |
...
他: posting_queue, product_packages, note_drafts, sns_engagement, progress_log, failure_memory, skill_registry, sprint_contracts, daichi_dialogue_log, session_memory等を含め50+テーブル。
...
*SYUTAINβ 完全設計書 V28 — 2026-04-01作成*
*プロジェクトオーナー: 島原大知*
