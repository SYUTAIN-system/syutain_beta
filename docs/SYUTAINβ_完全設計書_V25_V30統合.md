...
| V25（V30統合版 rev.5） | 2026-04-06 | 島原大知 × Claude Opus 4.6 | **Grok 活用 9 機能の全システム展開** (rev.4 で X リアルタイム検索の下地を作った直後、島原さんから 10 活用案のうち #4 リプライ下書きアシストを除く 9 機能の一括実装指示): (1) **新規モジュール `tools/grok_helpers.py`** — 9 ヘルパー関数を集約。内部 `_call_grok_json` 共通ラッパで Responses API 呼出・JSON パース・code fence 除去・コスト記録を統一。(2) **#1 記事ファクト強化** — `content_pipeline.py` Stage 3.5 で初稿から年号/%/金額含む文を最大5件抽出し `grok_fact_check` で X/Web 照合、虚偽 2 件以上で draft 棄却。(3) **#2 話題接続検証 (軽量版)** — 当初は投稿ごと `grok_topic_traction` を想定したがコスト高のため、**バッチ開始前に 1 回だけ `grok_trending_hashtags` を呼び出して persona_hint に注入**する #5 と統合した設計に変更。`sns_batch.py` で intel 取得後に実施、全候補投稿が共通のハッシュタグプールを参照。(4) **#3 `grok_competitor_monitor`** — `scheduler.py` 毎日06:00 JST。SYUTAINβ / 島原大知 / Build in Public 関連キーワード 8 個の X言及を追跡、importance='high' の mention は event_log + Discord 通知。(5) **#5 `grok_trending_hashtags`** — SNS バッチ先頭で 1 回実行 (~¥2-3)、6 件のタグを persona_hint に「今 X で実際に使われているハッシュタグ」として注入、全投稿で共有。(6) **#6 `grok_topicality_score`** — `content_pipeline.py` Stage 1.6 (ネタ選定直後) で時事性 0-1 スコアと urgency (immediate/today/this_week/anytime) を判定、stages に記録。(7) **#7 `grok_incident_research`** — `scheduler.py` 毎日06:30 JST。直近24hの event_log error/critical を集約して Grok に類似事例・解決策を問い合わせ、high relevance な case があれば Discord 通知 + event_log に 'grok.incident_research' 記録。(8) **#8 `grok_persona_verify_loop`** — `scheduler.py` 月曜07:00 JST 週次。直近7日の `daichi_dialogue_log` から発言を最大5件取り、島原さんの公開情報と照合して矛盾候補を検出、発見時 'grok.persona_verify' として event_log 保存。(9) **#9 `grok_market_sentiment`** — `agents/proposal_engine.py` の Revenue Scoring 後、total_score が AUTO_PROPOSAL_THRESHOLD-10 以上のときだけ追加呼出。go_no_go='no_go' or red_flags 2 件以上で is_auto_recommendable を False に降格。proposal dict に market_sentiment フィールドを追加。(10) **#10 `grok_upcoming_events`** — `scheduler.py` 日曜21:00 JST 週次。今後7日間の製品発表・イベント・話題ピークを島原関連6分野で予測し、intel_items に source='grok_upcoming_events', category='content_calendar', review_flag='actionable' で保存。翌週の note 記事 / SNS 投稿の theme candidate に組み込まれる。**#4 リプライ下書きアシスト** は島原さん指示で今回のスコープから除外。(11) **実機検証** — `grok_topicality_score` (Claude Code Skills テーマ)=score 0.90 urgency today ¥3.56、`grok_market_sentiment` (commission_article feature pitch)=positive conditional ¥4.25、`grok_trending_hashtags` (AI映像制作)=5件 ¥2.62 — いずれも `cost_in_usd_ticks` 経由で実コスト確認。(12) **コスト予算** — rev.4 の X リサーチ朝夕2回 (~¥4-6/日) に加え、rev.5 の 4新規ジョブ + 3インライン統合で **約 ¥15-30/日追加**、合計でも日次予算 ¥120 の 25% 程度に収まる見込み。全ジョブに budget_guard.check_before_call 事前チェックあり。(13) **scheduler 6 Grok ジョブ登録確認** — 06:00 (競合) / 06:30 (障害) / 月曜07:00 (ペルソナ) / 08:30 (X朝) / 19:30 (X夕) / 日曜21:00 (カレンダー)、実ログで next_run_at 確認済み。 |

---

最終更新：2026年4月6日
設計者：島原大知 × Claude Opus 4.6
バージョン：V25（V30統合版 rev.5）
