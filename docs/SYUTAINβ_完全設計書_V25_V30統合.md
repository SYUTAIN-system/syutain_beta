...
> **V30テーマ：「コードを一行も書けない人間が、AIエージェントと共に自律型事業OSを構築し、その全過程を公開するドキュメンタリー」** — Build in Public + GitHub公開 + 6層品質防御 + 27Bモデル統合 + intel活用パイプライン

**バージョン：** V25（V30統合版・2026年4月4日）
...
| 3 | モデル戦略 | Qwen3.5-9B主力 + Gemini 3.1 Pro + GPT-5.4 | **GPT-5.4（Computer Use/Tool Search/1M ctx）正式統合 + DeepSeek V4監視 + BRAVO qwen3.5:27b（Tier L+）** |
...
| ローカルLLM | **Qwen3.5-9B（Ollama、RTX 5070 12GB VRAM で余裕動作）+ Qwen3.5-27B（17GB、GPU+CPUオフロード、5 tok/s）** |
...
BRAVO（Ollama）+ CHARLIE（Ollama）の2台が常時並列推論の主力。ALPHAにはローカルLLMが存在しない（2026-03-06アンインストール済み）。DELTAは軽量タスク専用で安定稼働を維持。BRAVO上のqwen3.5:27bはhighest_local品質要求時のみ使用。
...
| V25（V30統合版） | 2026-04-04 | 島原大知 × Claude Opus 4.6 | ALPHA LLM撤去、BRAVO 27B追加、KV Cache Q8全ノード、Build in Public方針、6層品質防御、SNS拡散力強化、英語記事取り込み、GitHub公開セキュリティ、夜間モード拡張（23:00-09:00）、月額予算¥2,000、note_draft_generation統合、統計更新（52,325行Python/132ファイル）、スケジューラ70ジョブ |
