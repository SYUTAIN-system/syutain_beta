...
| BRAVO | Ryzen + RTX 5070 12GB | Ubuntu | 100.75.146.9 | Qwen3.5-9B（Ollama常駐） | RTX 5070 12GB | 推論優先ノード / Browser / ComputerUse |
| CHARLIE | Ryzen 9 + RTX 3080 10GB | Ubuntu（Win11デュアルブート） | 100.70.161.106 | Qwen3.5-9B（Ollama常駐） | RTX 3080 10GB | 推論ノード / コンテンツ生成 |
| DELTA | Xeon E5 + GTX 980Ti 6GB / 48GB RAM | Ubuntu | 100.82.81.105 | qwen3:4b-q4_K_M（Ollama常駐） | GTX 980Ti 6GB | モニタリング / 情報収集 / 突然変異エンジン |
...
│  ┌──────────┐  ┌──────────┐  ┌──────────────┐                 │
│  │  BRAVO   │  │ CHARLIE  │  │    DELTA     │                 │
│  │100.75.146│  │100.70.161│  │ 100.82.81.105│                 │
│  │          │  │          │  │              │                 │
...
cluster {
  ...
  listen: "0.0.0.0:6222"
  routes: [
    "nats-route://100.75.146.9:6222",
    "nats-route://100.70.161.106:6222",
    "nats-route://100.82.81.105:6222"
  ]
}
```
...
```
daichimac-mini.tail3d7a2e.ts.net:8443 {
    tls internal
...
接続: ws://localhost:8000/api/chat/ws?token=<jwt>
     または wss://daichimac-mini.tail3d7a2e.ts.net:8443/api/chat/ws?token=<jwt>
...
```
BRAVO (100.75.146.9):
  http://100.75.146.9:11434/api/generate  (Ollama)
  モデル: qwen3.5:9b (または互換名)

CHARLIE (100.70.161.106):
  http://100.70.161.106:11434/api/generate (Ollama)
  モデル: qwen3.5:9b

DELTA (100.82.81.105):
  http://100.82.81.105:11434/api/generate  (Ollama)
  モデル: qwen3:4b-q4_K_M
...
```
CHARLIE (100.70.161.106) Ubuntu稼働時:
  - worker_main.py 起動
  - ContentWorker: コンテンツ生成タスク処理
  - Ollama: qwen3.5:9b 常駐
  - NATS接続: 100.75.146.9:4222（ALPHA経由）
...
- ALPHA ↔ BRAVO (100.75.146.9): Tailscale暗号化トンネル
- ALPHA ↔ CHARLIE (100.70.161.106): Tailscale暗号化トンネル
- ALPHA ↔ DELTA (100.82.81.105): Tailscale暗号化トンネル
...
|--------|-------------|-----------------|
| CHARLIE | 100.98.82.108 | 100.70.161.106 |
| DELTA | 100.99.122.69 | 100.82.81.105 |

BRAVO (100.75.146.9) は設計書と一致。
...
# Web UI（ブラウザ）
open https://daichimac-mini.tail3d7a2e.ts.net:8443
```
...
原因: DELTA (100.82.81.105) へのTailscale経路が不安定
解決:
  # DELTAのNATS接続は断続的だが、WorkerはHTTP fallbackで動作継続
  # DELTAへの直接HTTP確認
  curl http://100.82.81.105:8001/health
  # DELTAのNATSルート再接続
...
確認:
  ...
  curl http://100.75.146.9:11434/api/tags  # BRAVO
  curl http://100.70.161.106:11434/api/tags  # CHARLIE
  # モデルが存在するか確認
  # → qwen3.5:9b が表示されるはず
...
**状況:** DELTA (100.82.81.105) へのNATSルート接続が断続的にタイムアウトする。
...
NODE_BRAVO_HOST=100.75.146.9
NODE_CHARLIE_HOST=100.70.161.106
NODE_DELTA_HOST=100.82.81.105
