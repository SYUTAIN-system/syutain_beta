...
## 最重要方針: Build in Publicドキュメンタリー（2026年4月2日決定）
note記事のテーマは「SYUTAINβで実際に何が起きたか」が最優先。
- 外部AIニュース解説記事（「GPTの使い方」「Claude活用法」等）は禁止
- SYUTAINβの実運用データ・実失敗・実メトリクスに基づく記事のみ
- 素材: PostgreSQLの実数値、LoopGuard発動ログ、エラー履歴、コスト推移、SNS投稿統計
- 「AIツールの紹介」ではなく「AIシステムを運用して実際に何が壊れたか、何を学んだか」
- 6月1日まで全記事無料（リーチ拡大フェーズ）
- 情報収集（intel_items）は記事の「補足素材」として使用可。メインテーマにはしない

## 戦略コンテキスト
{strategy_context}

## 直近の情報収集（補足素材として活用可能。メインテーマにはしない）
{intel_context}
{persona_context}
## 現在の状況
{context}
...
## 絶対に守るルール
1. 「島原が今日取れる具体的な1アクション」を必ず含めること（例: 「noteに記事を1本書く」「SNSで実運用データを共有する」「Build in Public投稿を3件生成する」）
2. 過去に却下された提案と同じテーマ・構造は禁止
3. 所要時間と想定コストを具体的に書くこと
4. 「連載」「シリーズ」より「単発で完結する1本」を優先
5. note記事の提案は必ずSYUTAINβの実データ・実体験に基づくこと。外部AIニュースの解説・紹介記事は却下される
6. テーマ例: 「LoopGuardが1日54回発動した話」「月936円で10,170回のLLM呼び出しを回す設計」「セマンティックループ検知器がシステム全体を止めた日」
7. 【2026年6月1日まで厳守】全記事無料公開・Build in Publicフェーズ。Booth/有料販売/価格設定/課金発生の提案は一切禁止。提案に「Booth」「有料販売」「課金」「価格設定」を含めたら即却下される
8. 拡散フェーズ: 提案には「拡散KPI改善への寄与」（インプレッション↑、フォロワー↑、エンゲージメント↑）を明記すること
...
{{
  ...
  "scoring": {{
    ...
    "continuity_value": 0-10
        ...
        # Revenue Score を計算（上限値を強制適用 — LLMの自己採点インフレを防止）
        scoring = proposal_data.get("scoring", {})
        _SCORE_CAPS = {
            "icp_fit": 25, "channel_fit": 15, "content_reuse": 15,
            "speed_to_cash": 15, "gross_margin": 10, "trust_building": 10,
            "continuity_value": 10,
        }
        for key, max_val in _SCORE_CAPS.items():
            if key in scoring:
                scoring[key] = min(int(scoring[key]), max_val)
        total_score = sum(scoring.get(k, 0) for k in _SCORE_CAPS.keys())
        # 100点満点を超えないことを保証
        total_score = min(total_score, 100)
        is_auto_recommendable = total_score >= AUTO_PROPOSAL_THRESHOLD

        # Grok #9: 意思決定前リアルタイム市場反応検索
        # 自動推奨閾値近傍のときだけ Grok に追加確認（コスト抑制）
        market_sentiment: dict = {}
        try:
            if total_score >= (AUTO_PROPOSAL_THRESHOLD - 10):
                from tools.grok_helpers import grok_market_sentiment
                proposal_summary = (
                    f"{proposal_data.get('title', '')}\n"
                    f"目的: {objective} / ICP: {target_icp} / チャネル: {primary_channel}\n"
                    f"理由: {'; '.join((proposal_data.get('why_now', []) or [])[:3])}\n"
                    f"期待成果: {proposal_data.get('expected_outcome', {})}"
                )[:1500]
                ms = await grok_market_sentiment(proposal_summary)
                if ms.get("ok"):
                    market_sentiment = {
                        "sentiment": ms.get("sentiment", "neutral"),
                        "confidence": ms.get("confidence", 0.5),
                        "go_no_go": ms.get("go_no_go", "conditional"),
                        "red_flags": ms.get("red_flags", []),
                        "differentiation": ms.get("differentiation_opportunities", []),
                        "reasoning": ms.get("reasoning", "")[:300],
                        "cost_jpy": ms.get("cost_jpy", 0.0),
                    }
                    logger.info(
                        f"Grok市場反応: sentiment={market_sentiment['sentiment']} "
                        f"go_no_go={market_sentiment['go_no_go']} ({market_sentiment['cost_jpy']:.2f}円)"
                    )
                    # red_flags が 2件以上、または go_no_go='no_go' の場合は auto 推奨を外す
                    if ms.get("go_no_go") == "no_go" or len(ms.get("red_flags", [])) >= 2:
                        if is_auto_recommendable:
                            is_auto_recommendable = False
                            logger.warning(f"Grok market sentiment で auto_recommend 却下: {market_sentiment['red_flags']}")
        except Exception as grok_err:
            logger.debug(f"Grok market sentiment スキップ: {grok_err}")
        ...
        proposal = {
            ...
            "market_sentiment": market_sentiment,  # Grok #9 追加
...
## 最重要: Build in Public方針チェック
この提案が以下に違反していないか必ず確認すること:
- note記事のテーマは「SYUTAINβで実際に何が起きたか」が最優先
- 外部AIニュース解説記事（「GPTの使い方」等）は禁止
- 未リリースのモデル（推測段階のもの）を題材にした提案は禁止
- 6月1日まで有料販売の提案は不適切
違反がある場合、risks の最初の項目に「Build in Public方針違反」と明記すること。
...
## 最重要: Build in Public方針
代替案は「SYUTAINβで実際に何が起きたか」に基づくこと。
外部AIニュース解説やツール紹介の代替案は禁止。
6月1日まで全記事無料（有料販売は6月以降）。
