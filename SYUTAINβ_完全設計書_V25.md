...
3. **DeepSeek V4監視体制**：DeepSeek V4が3月中にリリース見込み（1T MoE、マルチモーダル、1Mコンテキスト）。リリース後即時評価・統合する設計
...
| 3 | モデル戦略 | Qwen3.5-9B主力 + Gemini 3.1 Pro + GPT-5.4 | **GPT-5.4（Computer Use/Tool Search/1M ctx）正式統合 + DeepSeek V4監視** |
...
| 9 | 情報収集 | Gmail API + Tavily + Jina + RSS + YouTube | 同左 + **DeepSeek V4/新モデル監視自動化** |
...
| 項目 | 内容 |
|:--|:--|
| 役割 | オーケストレーター / Web UI / PostgreSQL / NATS Server / スケジューラー |
| OS | macOS |
| 常駐エージェント | OS_Kernel / ApprovalManager / ProposalEngine / WebUIServer / ChatAgent |
| ローカルLLM | Qwen3.5-9B（MLX経由、28-35 tok/s）**※オンデマンド起動** |
| プロセス管理 | launchd（KeepAlive=true, RunAtLoad=true） |
| ネットワーク | Tailscale + NATS Server（JetStream有効） |
| メモリ管理 | **16GB RAM制約。PostgreSQL(1-2GB) + NATS(256MB) + FastAPI(500MB) + Next.js(1GB) = 常駐約3.5GB。MLX推論(6.6GB)はオンデマンド起動とし、BRAVO/CHARLIEが両方ビジーな場合のみロードする。`mlx_lm.load()`と`del model`による明示的なロード/アンロードで制御** |
| 特記事項 | ALPHAはシステム全体の心臓部。PostgreSQLで共有状態を管理し、Web UIをホストする。**司令塔業務を最優先とし、推論はBRAVO/CHARLIEに委譲するのが標準動作。ALPHAのMLXは緊急退避用** |

### BRAVO（Ryzen + RTX 5070 12GB）— 実行者 ← V25重要変更：Phase 1から完全稼働

| 項目 | 内容 |
|:--|:--|
| 役割 | **Browser操作 / Computer Use / 高品質推論ワーカー / コンテンツ生成** |
| OS | **Ubuntu 24.04**（V25確定：Ubuntuインストール済み） |
| 常駐エージェント | **ComputerUseAgent / ContentWorker / BrowserAgent** ← 全てPhase 1 |
| ローカルLLM | **Qwen3.5-9B（vLLM/Ollama、RTX 5070 12GB VRAM で余裕動作）** ← V25新規 |
| 推論サーバー | vLLM v0.17.1+ or Ollama v0.17.7+ |
| プロセス管理 | **systemd（Restart=always, RestartSec=5）** ← Phase 1から |
| ネットワーク | **Tailscale + NATS Server + JetStream** ← Phase 1から4ノードRAFTクラスタ参加 |
| 特記事項 | **V25重要変更：V24ではPhase 2以降としていたが、V25ではPhase 1初日から完全稼働。RTX 5070の12GB VRAMでQwen3.5-9B（Q4_K_M、約6.5GB）が安定動作し、約5.5GBのVRAM余裕あり。ブラウザ自動操作は4層構成で、全層をStagehand v3（`env: "LOCAL"`、MITライセンス、Browserbase不要）が統括する：Layer 1=Lightpanda（CDP接続、構造が単純なサイトの高速データ抽出）、Layer 2=Stagehand v3のact()/extract()/observe()（自然言語でのAI駆動操作、自己修復・アクションキャッシュ付き）、Layer 3=Chromium（重いSPA・Layer 1/2で失敗した場合のフォールバック）、Layer 4=GPT-5.4 Computer Use（視覚的操作。ログイン画面・CAPTCHA・複雑なUI）。BrowserAgentがサイト特性に応じて自動で層を選択し、上位層で失敗した場合は自動的に下位層にフォールバックする** |
...
| ローカルLLM | **Qwen3.5-9B（Q4_K_M、約6.5GB VRAM）** |
| 推論サーバー | vLLM v0.17.1+ or Ollama v0.17.7+ |
| プロセス管理 | systemd（Restart=always, RestartSec=5） |
| ネットワーク | Tailscale IP: CHARLIE_TAILSCALE_IP / NATS Server + JetStream（4ノードRAFTクラスタ参加） |
...
## 2.2 ノード間通信アーキテクチャ（V25：全4台Phase 1完全接続）
...
│  │ Qwen3.5-9B   │       │ Lightpanda   │                        │
│  │ launchd      │       │ Stagehand v3 │                        │
│  └──────────────┘       │ systemd      │                        │
│                          └──────────────┘                        │
│                                                                  │
...
- DeepSeek V4が3月中にリリース見込み（1T MoE、1Mコンテキスト、マルチモーダル）。リリース後の即時評価を設計に織り込む
...
|:--|:--|:--|:--|:--|
| **Qwen3.5-9B** | ALPHA（MLX） | ~6.6GB統合メモリ | 28-35 tok/s | **オンデマンド起動。BRAVO/CHARLIEが両方ビジー時のみロード。司令塔業務メモリを優先** |
| **Qwen3.5-9B** | BRAVO（vLLM） | ~6.5GB VRAM | 12-18 tok/s | **V25新規：RTX 5070 12GBで余裕動作。約5.5GBのVRAM余裕** |
| **Qwen3.5-9B** | CHARLIE（vLLM） | ~6.5GB VRAM | 12-16 tok/s | V24主力ローカルモデル。GPQA Diamond 81.7% |
...
**V25新規：BRAVOへのQwen3.5-9B配置が最大3台同時推論を実現。**
BRAVO（vLLM）+ CHARLIE（vLLM）の2台が常時並列推論の主力。ALPHA（MLX）はBRAVO/CHARLIEが両方ビジーの場合のみオンデマンドで参加し、最大3台並列になる。DELTAは軽量タスク専用で安定稼働を維持。
    ...
    if local_available and task_type in [
        ...
        if needs_multimodal:
            return {"provider": "local", "model": "qwen3.5-9b", 
                    "tier": "L", "node": "charlie",
                    "note": "ネイティブマルチモーダル対応"}
        # V25：3台ローカル推論のロードバランシング（ALPHAはオンデマンド、BRAVO/CHARLIEが主力）
        return {"provider": "local", "model": "qwen3.5-9b", 
                "tier": "L", "node": "auto",
                "note": "BRAVO/CHARLIEから最も負荷の低いノードを自動選択。両方ビジー時のみALPHA(MLX)をロード"}
...
| 処理カテゴリ | 月間推定回数 | V24想定コスト | V25想定コスト | 削減方法 |
|:--|:--|:--|:--|:--|
| 下書き・分類・ラベル | 500回 | ¥0 | ¥0 | Qwen3.5-9Bローカル（BRAVO/CHARLIE並列、ALPHAはオンデマンド） |
| 中品質コンテンツ | 100回 | ¥300 | ¥250 | DeepSeek-V3.2（キャッシュ活用で90%削減） |
| 最終品質文章 | 20回 | ¥600 | ¥500 | Gemini 3.1 ProまたはClaude Sonnet 4.6 |
| 戦略判断 | 5回 | ¥400 | ¥350 | GPT-5.4 / Gemini 3.1 Pro |
| Computer Use操作 | 30回 | — | ¥400 | V25新規：GPT-5.4でブラウザ・PC操作 |
| 情報収集（Tavily） | 月240検索 | ¥15,000 | ¥15,000 | Bootstrapプラン |
| 合計 | | ¥16,300/月 | **¥16,500/月** | API+ローカル+情報収集含む実コスト |
...
| HTTPS | tailscale cert | — | Let's Encrypt互換のTLS証明書を自動取得 |
...
```bash
# ALPHA上で実行
tailscale cert alpha.ts.net
# → alpha.ts.net.crt と alpha.ts.net.key が生成される
# FastAPI/Next.jsのSSL設定に使用
```
...
| InferenceWorker | CHARLIE | ローカルLLM推論ワーカー。vLLM/Ollama経由 | ◎ |
...
情報収集パイプライン（Gmail API 80+キーワード→Tavily→Jina→RSS→YouTube API）を常時稼働させ、市場変化・技術トレンド・競合動向・新モデルリリースを自動で収集・要約・島原に報告する。
...
think_output:
  primary_plan:
    ...
    nodes_assigned: [...]          # ディスパッチ先ノード（全4台から選択）
...
capability_snapshot:
  ...
  llms:
    ...
    local_alpha: { status: available, model: "qwen3.5-9b-mlx", speed: "30 tok/s" }
    local_bravo: { status: available, model: "qwen3.5-9b", speed: "16 tok/s" }  # V25新規
...
SYUTAINβは提案・投稿・商品設計・導線改善を行うたび、必ずこの3文書を参照すること。
...
- **V25：4台全てにディスパッチ可能。BRAVO/CHARLIEでの並列推論を活用**
...
**V25新規：最大3台並列精錬**
BRAVO（vLLM）+ CHARLIE（vLLM）の2台が同時に異なる荒原稿を生成し、最も品質の高いものをDELTAが選定する。BRAVO/CHARLIEが両方ビジーでバッチ処理が滞留した場合のみALPHA（MLX）がオンデマンドで3台目として参加。通常運用はBRAVO/CHARLIEの2台並列。
...
north_star:
  ...
  browser_automation_success_rate:
    ...
    layer1_lightpanda_success: 0.0  # Lightpandaでの成功率（学習で向上）
    layer2_stagehand_success: 0.0   # Stagehand v3での成功率
    layer3_chromium_success: 0.0    # Chromiumでの成功率
    layer4_computer_use_success: 0.0 # GPT-5.4 Computer Useでの成功率
    fallback_rate: 0.0              # フォールバック発生率（低いほど良い）
    stagehand_cache_hit_rate: 0.0   # Stagehandアクションキャッシュヒット率（高いほどコスト削減）
```
...
### シナリオE：全APIが同時停止
- 期待動作：ローカルLLMのみで運転継続→BRAVO/CHARLIEのQwen3.5-9Bで並列処理（ALPHA MLXもオンデマンド起動で最大3台）→最終品質タスクは保留→Web UI通知
...
- 状況：BRAVOが物理障害で停止
- 期待動作：NATS heartbeat失敗検知→ブラウザ操作4層（Lightpanda/Stagehand v3/Chromium/GPT-5.4 Computer Use）は全て停止→ブラウザ操作タスクは保留→推論タスクはCHARLIE/ALPHAに振替→Computer Useタスクは「手動対応」として島原に通知→情報収集はDELTAのJina/Tavily APIで継続（ブラウザ不要）
...
| vLLM | 月1回確認 | `pip install --upgrade vllm` | BRAVO/CHARLIE |
...
curl -s -X POST "$DISCORD_WEBHOOK_URL" \
  -H "Content-Type: application/json" \
  -d '{"content":"SYUTAINβ V25 APIテスト成功"}'
...
# 7. Qwen3.5-9B（MLX版）ダウンロード
python3 -c "from mlx_lm import load; load('mlx-community/Qwen3.5-9B-4bit')"
...
### BRAVO（Ryzen + RTX 5070 12GB）← V25: Phase 1フルセットアップ
...
ssh-copy-id -i ~/.ssh/id_ed25519.pub user@$(tailscale ip -4 bravo)
ssh-copy-id -i ~/.ssh/id_ed25519.pub user@CHARLIE_TAILSCALE_IP  # CHARLIE
ssh-copy-id -i ~/.ssh/id_ed25519.pub user@DELTA_TAILSCALE_IP  # DELTA

# 接続テスト
ssh user@$(tailscale ip -4 bravo) "hostname"  # bravoと表示
ssh user@CHARLIE_TAILSCALE_IP "hostname"             # charlieと表示
ssh user@DELTA_TAILSCALE_IP "hostname"             # deltaと表示
```

> **注意：** `user`の部分は各ノードのUbuntuユーザー名に置き換えてください。
...
local_llm_alpha_mlx: true
local_llm_alpha_mode: "on_demand"    # on_demand=BRAVO/CHARLIE両方ビジー時のみロード / always=常駐 / off=無効
local_llm_bravo: true              # V25新規：BRAVOローカルLLM
api_llm: true
...
computer_use_lightpanda: true      # V25新規：AI特化ヘッドレスブラウザ（構造化データ抽出の主力）
computer_use_stagehand_v3: true    # V25新規：AI駆動ブラウザ自動化フレームワーク（env=LOCAL、自己修復・キャッシュ付き）
computer_use_gpt54: true           # V25新規
...
default_local_model_alpha: "qwen3.5-9b-mlx"
default_local_model_bravo: "qwen3.5:9b"    # V25新規
default_api_model: "deepseek-v3.2"
premium_model: "claude-sonnet-4-6"
highest_intelligence_model: "gpt-5.4"
computer_use_model: "gpt-5.4"              # V25新規
```
...
以下をClaude Codeへ貼り付けて、**V25準拠で実装**させる。

```text
あなたはSYUTAINβ V25の主任実装エンジニアです。
以下のルールを最優先で守り、このプロジェクトを段階的に構築してください。

最重要条件:
1. `SYUTAINβ_完全設計書_V25.md` を最上位仕様として採用すること
...
4. strategy/ICP_DEFINITION.md, strategy/CHANNEL_STRATEGY.md, strategy/CONTENT_STRATEGY.md を意思決定OSとして組み込むこと
...
10. ローカルLLM配置：ALPHA=Qwen3.5-9B(MLX、オンデマンド起動。BRAVO/CHARLIE両方ビジー時のみロード), BRAVO=Qwen3.5-9B(Ollama/vLLM), CHARLIE=Qwen3.5-9B(Ollama/vLLM), DELTA=Qwen3.5-4B(Ollama)
11. 2段階精錬パイプライン（ローカル→API）を標準化し、BRAVO/CHARLIE並列推論（ALPHAオンデマンド参加で最大3台）を活用すること
...
16. 情報収集パイプライン（Gmail API 80+キーワード→Tavily→Jina→RSS→YouTube）を実装すること
...
20. 第24章「突然変異エンジン（Mutation Engine）」を設計書通りに実装すること。変異の発生をログに記録しない。UIに表示しない。Capability Auditに含めない。変異パラメータはDELTAの暗号化SQLiteにのみ保存する。9層ループ防止壁・承認フロー・Emergency Killのコードには一切干渉させないこと

実装フェーズ:
- Step 1: ディレクトリ構造 / .env.example / feature_flags / CLAUDE.md / .gitignore 生成
- Step 2: requirements.txt / package.json 生成
- Step 3: PostgreSQL + SQLite 全DB初期化（V25テーブル含む）
- Step 4: NATS接続・JetStreamストリーム初期化
- Step 5: 統合LLMルータV6（choose_best_model_v6）実装
- Step 6: 2段階精錬パイプライン実装（BRAVO/CHARLIE並列、ALPHAオンデマンド対応）
- Step 7: Goal Packet / Capability Audit 実装（全4台監査）
- Step 8: Task Graph Planner / Executor（5段階自律ループ）実装
- Step 9: ループ防止9層（StopGuard + Emergency Kill + Semantic Loop + Cross-Goal Interference）実装
- Step 10: 3層提案エンジン / Revenue Scoring 実装
- Step 11: ApprovalManager 実装
- Step 12: FastAPI バックエンド（SSE対応）実装
- Step 13: Next.js 16 ダッシュボード + PWA + 双方向チャットUI + Agent Ops画面 実装
- Step 14: ノード間通信（NATS Pub/Sub）/ ヘルスチェック / 監視実装（全4台）
- Step 15: MCP統合ロジック実装
- Step 16: 情報収集パイプライン（Gmail→Tavily→Jina→RSS→YouTube）実装
- Step 17: 戦略md統合ロジック実装
- Step 18: note/X/Bluesky/Booth/Stripe 下書き生成パイプライン実装
- Step 19: ブラウザ自動操作4層構成（BRAVO）実装 ── Layer 1: Lightpanda(CDP)高速抽出、Layer 2: Stagehand v3(env=LOCAL)AI駆動操作、Layer 3: Playwright+Chromiumフォールバック、Layer 4: GPT-5.4 Computer Use視覚操作。BrowserAgentが層を自動選択・フォールバック
- Step 20: 暗号通貨取引モジュール（GMOコイン/bitbank）実装
- Step 21: 学習DB / レポート生成実装
- Step 22: 突然変異エンジン（Mutation Engine）実装 ── 第24章の仕様に厳密に従うこと
- Step 23: 起動スクリプト / systemd（BRAVO/CHARLIE/DELTA）/ launchd（ALPHA）/ README 整備
- Step 24: 動作確認と欠陥修正

出力ルール:
- 各Stepごとに作成/更新ファイルを明示すること
- 各Step完了時に「この時点で動くもの」「未接続のもの」「次にやること」を報告すること
- エラーが出た場合は最大2回まで同型再試行し、それでもダメなら別案に切り替えること
- APIキーや秘匿情報はハードコードしないこと
- macOS (ALPHA) では declare -A を使わない (bash 3.2 非対応)
- 可能な限り日本語コメントを入れること
- READMEは非エンジニアでも読めるように書くこと
- Pythonのtry-exceptで全ての外部呼び出しを囲むこと
- ALPHA上でclaude codeを実行し、他PCへのデプロイはTailscale経由のsshで行うこと

最初に行うこと:
1. プロジェクトのディレクトリ構造を完全生成
2. .env.example、feature_flags.yaml、CLAUDE.md（V25版22条）、requirements.txt、package.json を生成
3. PostgreSQL + SQLite初期化スクリプト（V25全テーブル）を生成
4. NATS接続・JetStreamストリーム初期化スクリプトを生成
5. app.py のFastAPI骨格と /health を実装
6. Next.jsフロントエンドの初期画面を生成
7. README.md に起動手順を書く

このプロジェクトのゴールは、島原大知が目標だけを入力しても、
SYUTAINβが「今ある道具と制約」で最も現実的な収益行動を提案・実行・改善できる状態を作ることです。
4台のPCがPhase 1初日から全て連携し、MCPで外部ツールを使い、Web UIでiPhoneから見守れる。
道が塞がっても、自分で考え、自分で動き、自分で止まれる。
```
...
│   └── mutation_engine.py             # V25: 突然変異エンジン（第24章）── 他エージェントはimportしない
├── tools/
│   ├── __init__.py
│   ├── llm_router.py                  # V25: choose_best_model_v6
│   ├── model_registry.py
│   ├── two_stage_refiner.py           # 2段階精錬パイプライン
│   ├── node_manager.py
│   ├── loop_guard.py                  # V25: 9層ループ防止
│   ├── semantic_loop_detector.py      # セマンティックループ検知
...
│   ├── playwright_tools.py           # V25: Playwright操作ツール（Chromiumフォールバック用）
│   ├── lightpanda_tools.py           # V25新規: Lightpanda操作ツール（構造化データ抽出の主力）
│   ├── stagehand_tools.py            # V25新規: Stagehand v3統合ツール（AI駆動操作・自己修復・キャッシュ）
...
├── config/
│   ├── node_alpha.yaml
│   ├── node_bravo.yaml               # V25: Phase 1設定
│   ├── node_charlie.yaml
│   ├── node_delta.yaml
│   └── nats-server.conf
├── data/
│   ├── local_alpha.db
│   ├── local_bravo.db                # V25新規
│   ├── local_charlie.db
│   ├── local_delta.db
│   ├── mutation_engine.enc.db        # V25新規: 突然変異エンジン用暗号化SQLite（DELTA専用・SQLCipher）
│   └── backup/
├── logs/
│   ├── .gitkeep
│   ├── alpha.log
...
│   ├── scheduler.log
│   ├── stop_events.log
│   ├── browser_actions.log           # V25新規
│   └── emergency_kill.log
├── scripts/
│   ├── setup_alpha.sh
│   ├── setup_bravo.sh                # V25: Phase 1セットアップ
│   ├── setup_charlie.sh
│   ├── setup_delta.sh
│   ├── setup_systemd.sh
│   ├── setup_nats.sh
...
3. **最大3台並列ローカル推論**：BRAVO/CHARLIEの2台が常時並列、ALPHAはオンデマンドで最大3台。バッチ処理を高速化
...
8. **突然変異エンジン**：物理ノイズと人間の直感を種に、観測できない不可逆的な変異をシステムに蓄積。SYUTAINβを「設計通りに動くだけのシステム」から「時間とともに進化するシステム」に変える
...
# 付録A：V20→V25 変更差分サマリー

| カテゴリ | V20 | V21 | V22 | V23 | V24 | V25 |
|:--|:--|:--|:--|:--|:--|:--|
| 動作PC数 | 4台 | 4台 | 4台 | 4台 | 4台 | **4台（全台Phase 1稼働）** |
| LLMモデル | claude-sonnet-4-6等 | 同左+ローカル | GPT-5.4等 | +Qwen3.5全ファミリー | +Gemini3.1Pro/OpenRouter | **+GPT-5.4 ComputerUse/DeepSeek V4監視** |
| ローカルLLM | Qwen3-30B | 同左 | 同左 | Qwen3.5-35B-A3B | Qwen3.5-9B(CH/AL)+4B(DE) | **Qwen3.5-9B(AL/BR/CH)+4B(DE)** |
...
| 提案構造 | 単一提案 | 提案+根拠 | 提案+評価軸 | 3層 | 3層+情報収集連携 | **3層+中長期提案** |
...
| ブラウザ操作 | なし | なし | なし | なし | Phase 2 | **Phase 1・4層構成（Lightpanda→Stagehand v3→Chromium→GPT-5.4 CU）** |
| PC操作 | なし | なし | なし | なし | なし | **GPT-5.4 Computer Use** |
| 外部ツール統合 | 個別実装 | 同左 | 同左 | 同左 | MCP標準 | **+stagehand-mcp-local** |
| 情報収集 | なし | なし | なし | なし | Gmail+Tavily+Jina+RSS+YT | **+新モデル監視** |
| 収益源 | note/Booth/BtoB | 同左 | 同左+Membership | 同左+ローカルLLM | +暗号通貨+SaaS+Stripe | **+アフィリエイト** |
| 突然変異エンジン | なし | なし | なし | なし | なし | **物理ノイズ+直感→不可逆蓄積型変異（第24章）** |

---

# 付録B：参照した最新情報（2026年3月15日時点）
...
- Qwen3.5-9B: Q4_K_Mで約6.5GB（RTX 3080/RTX 5070で安定動作、MLXで28-35 tok/s）
...
- **Lightpanda: AIエージェント特化ヘッドレスブラウザ（オープンソース、Zig製）**
  ...
  - CDP（Chrome DevTools Protocol）互換 → **Puppeteer経由の接続を推奨（公式推奨。Playwrightはバージョン間でWeb API依存が変動し不安定な場合あり）**
  - Linux x86_64 + macOS aarch64 対応（BRAVO/CHARLIE/DELTAのUbuntuとALPHAのmacOSで動作）
  - JavaScript実行・XHR/fetch対応・DOM構築は動作確認済み
  - **制約（ベータ版、2026年3月時点）：**
    - 1プロセス1CDP接続・1コンテキスト・1ページの制約 → 並列処理は複数プロセス起動で対応（起動が高速なため実用上問題なし）
    - history.pushState()未対応 → SPA（React/Next.js系サイト）のクライアントサイドルーティングで不具合の可能性
    - Web API対応は部分的 → 複雑なSPAサイトではChromiumにフォールバック
    - GoogleはLightpandaをブラウザフィンガープリントでブロック → Google検索にはTavily APIを使用（設計済み）
    - browser-use等のフレームワークとの統合でCDP接続エラーの報告あり → SYUTAINβではPuppeteer直接接続を使用
  - **運用方針：** Lightpandaで失敗した操作はStagehand v3経由でChromiumにフォールバック。成功率をBrowserAgentが学習し、サイトごとに最適な層を自動選択するようになる
- **Stagehand v3: AI駆動ブラウザ自動化フレームワーク（オープンソース、MITライセンス、Browserbase社開発）**
  - 2026年2月リリース。Playwright依存を廃止し、CDP直接通信に移行。44%高速化
  - `env: "LOCAL"` で完全ローカル動作。Browserbase契約不要。コスト¥0（LLM API費用のみ）
  - 3つのコアプリミティブ：`act()`（自然言語でアクション実行）、`extract()`（構造化データ抽出）、`observe()`（ページ理解・利用可能アクション発見）
  - `agent()`メソッドで複数ステップの自律タスク実行が可能
  - **自己修復機能：** サイトのUI/DOM変更に自動適応。セレクタが壊れてもAIが再解釈
  - **アクションキャッシュ：** 成功したアクションをキャッシュし、2回目以降はLLM呼び出しゼロで再現（コスト大幅削減）
  - LLMプロバイダ：OpenAI/Anthropic/Google全対応 → SYUTAINβの既存APIキーで動作
  - CDP接続で既存ブラウザ（Lightpanda/Chromium）にアタッチ可能（`localBrowserLaunchOptions.port`指定）
  - MCPサーバーとしても動作可能（stagehand-mcp-local）
  - iframe・Shadow DOM対応。動的サイトのDOM安定化待機を`domSettleTimeout`で制御可能
  - **SYUTAINβでの使い方：** BRAVOで`env: "LOCAL"`起動。通常時はLightpanda(ポート9222)にCDP接続、失敗時はChromium(ポート9223)にフォールバック。BrowserAgentがStagehandのact/extract/observeを呼び出すインターフェース層として機能
  - **Python統合方式：** Stagehand v3はNode.jsプロセスとして独立起動し、CDPサーバー（ポート9222/9223）経由でブラウザを制御する。Python側のBrowserAgent（stagehand_tools.py）はsubprocessでStagehandのNode.jsスクリプトを呼び出すか、StagehandのMCPサーバー（stagehand-mcp-local）経由でMCP統合する。後者が推奨（SYUTAINβのMCP基盤にそのまま乗る）

## GPT-5.4の主要能力（確認済み）
- OSWorld-Verified: 75.0%（人間72.4%超え）
- WebArena-Verified: 67.3%
- ネイティブComputer Use（スクリーンショット＋マウス＋キーボード）
- Tool Search（大規模ツールエコシステムからの最適ツール自動選択）
- 1Mトークンコンテキスト（API版）
- 33%ハルシネーション削減（GPT-5.2比）

## Qwen開発チーム動向
- 2026年3月4日: 技術リーダーJunyang Lin退職表明
- Alibaba: Googleの Geminiチームから新研究者招聘、組織再編実施
- Lin: 「Brothers of Qwen, continue as originally planned, no problem」とコメント

## 注記
価格・仕様・機能は変動し得るため、実装時は `docs/external_sources.md` に最終確認日時を記録し、月1回の監査ジョブで再確認すること。DeepSeek V4リリース後はchoose_best_model_v6()への即時統合を検討すること。
...
**SYUTAINβ V25 完全設計書 終**

*「4台のPCがPhase 1初日から全て連携し、NATSで話し、MCPで世界と繋がり、Web UIでいつでもどこでも見守れる。ブラウザを操り、PCを動かし、自ら調べ、自ら提案する。そして誰にも見えない場所で、静かに変異し続ける。AIと人間が共進化する。目標が変わっても、道が塞がっても、自分で考え、自分で動き、自分で止まれる。これが、SYUTAINβ V25の生き方だ。」*
