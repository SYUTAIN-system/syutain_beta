...
class NoteQualityChecker:
    ...
    async def _check_article(self, filepath: Path) -> dict:
        ...
        try:
            ...
            if isinstance(ext_result, dict) and not ext_result.get("passed", True):
            ...
            logger.warning(f"外部検索ファクト検証エラー（安全側reject）: {e}")
            # 外部検証失敗時も安全側に倒す
            await self._insert_review(
                filepath, filename, title, len(content),
                final_status="rejected_external_fact",
                blocked_reason=f"外部検索検証エラー（安全側reject）: {e}",
            )
            return {
                "filepath": str(filepath), "title": title,
                "status": "rejected_external_fact",
                "mechanical": mech_result, "factual": factual_result,
                "haiku": None, "gpt5": None, "cost_jpy": article_cost,
            }
...
{content}"""
    ...
    async def _external_fact_verification(self, content: str, title: str) -> dict:
        ...
        - 「日本語記事がない/少ない」系の主張を検索で確認
        - トレンド検出結果の妥当性を検証
        - 既知のツール/技術についての誤った新規性主張を検出
        """
        import re
        issues = []

        # 1. 「日本語記事がない/少ない/ゼロ」系の主張を検出
        no_jp_patterns = re.findall(
            r'日本語(?:の)?(?:記事|情報|解説)(?:が)?(?:ない|なし|ゼロ|少ない|極端に少ない|ほぼ(?:ない|存在しない))',
            content
        )

        # 2. 主要な固有名詞（ツール/技術名）を抽出
        # タイトルと本文から「話題の」「注目の」等で修飾されたツール名を抽出
        ...
        # 検索が必要な場合のみ外部APIを呼ぶ
        if not no_jp_patterns and not tool_mentions:
        ...
        try:
            ...
            for tool_name in tool_mentions[:3]:  # 最大3つまで検証
                # ツール名で日本語記事を検索
                ...
                jp_results = [r for r in results if any(
                    c >= '\u3040' and c <= '\u9fff' for c in r.get("title", "")
                )]
                if jp_results and no_jp_patterns:
                    issues.append(
                        f"「{tool_name}」の日本語記事は実際に存在する"
                        f"（検索で{len(jp_results)}件発見: {jp_results[0].get('title', '')[:50]}）"
                    )

                # コスト記録
                cost = 2.0  # Tavily 1検索 ≈ ¥2
                await self._log_llm_cost("tavily-search", "note_external_fact_check", cost)
                self.run_cost += cost

        except Exception as e:
            logger.warning(f"外部検索ファクト検証でTavily呼び出し失敗: {e}")
            # Tavily失敗時はJina Searchにフォールバック
            try:
                from tools.jina_client import JinaClient
                jina = JinaClient()
                for tool_name in tool_mentions[:2]:
                    search_text = await jina.extract_markdown(f"https://search.jina.ai/search?q={tool_name}+使い方+日本語")
                    if search_text and len(search_text) > 100 and no_jp_patterns:
                        issues.append(
                            f"「{tool_name}」の日本語情報はJina検索で確認済み — 「日本語記事がない」は虚偽"
                        )
                    cost = 0.5
                ...
                # 両方失敗した場合、no_jp_patterns があれば安全側にreject
                if no_jp_patterns:
                    issues.append("「日本語記事がない」主張を外部検索で検証できなかった（安全側reject）")
