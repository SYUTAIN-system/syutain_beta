...
COST_LIMITS = {
    "max_cost_per_article_jpy": 15.0,     # 6→15: 外部検索ファクト検証分を考慮
    "max_articles_per_run": 5,
    "max_cost_per_run_jpy": 60.0,          # 30→60
    "max_articles_per_day": 10,
    "max_cost_per_day_jpy": 120.0,         # 60→120
    "max_cost_per_month_jpy": 1000.0,      # 500→1000
...
def mechanical_quality_check(content: str) -> dict:
    ...
    try:
        ...
        # === 13. 秘密情報チェック（content_redactor） ===
        try:
            from tools.content_redactor import is_safe_to_publish
            safe_to_pub, redact_issues = is_safe_to_publish(content)
            max_points += 1.0
            if safe_to_pub:
                score_points += 1.0
            else:
                issues.append(f"秘密情報検出: {len(redact_issues)}件 — 公開前に除去必須")
                # 秘密情報はD判定（即reject）
                return {
                    "score": 0.0,
                    "rank": "D",
                    ...
                    "details": {
                        ...
                        "unique_proper_nouns": unique_proper_nouns,
                        "meta_leaked": False,
                        "redact_issues": len(redact_issues),
                    },
                }
        except ImportError:
            pass  # content_redactor未インストール時はスキップ

        # === 14. タイトル健全性チェック ===
        max_points += 1.0
        title_line = lines[0].lstrip("#").strip() if lines else ""
        _title_problems = []
        # タスク記述がそのままタイトルになっていないか
        _task_leak_patterns = [
        ...
        for pat in _task_leak_patterns:
            if pat in title_line:
                _title_problems.append(f"タスク記述漏洩: '{pat}'")
        if len(title_line) > 80:
            _title_problems.append(f"タイトルが長すぎる: {len(title_line)}字（80字以内）")
        if len(title_line) < 10:
            _title_problems.append(f"タイトルが短すぎる: {len(title_line)}字")
        if _title_problems:
            issues.extend(_title_problems)
            # タスク記述漏洩は即D判定
            if any("タスク記述漏洩" in p for p in _title_problems):
                return {
                    "score": 0.0, "rank": "D", "issues": issues, "passed": False,
                    "details": {"title_problems": _title_problems, "meta_leaked": False},
                }
        else:
            score_points += 1.0
...
class NoteQualityChecker:
    ...
    async def _check_article(self, filepath: Path) -> dict:
        ...
        # === Stage 1.7: 外部検索によるファクト検証 ===
        ext_result = None
        search_evidence = []
        try:
            ext_result = await self._external_fact_verification(content, title)
            if isinstance(ext_result, dict):
                search_evidence = ext_result.get("evidence", [])
                if not ext_result.get("passed", True):
                    ext_issues = ext_result.get("issues", [])
                    logger.warning(f"外部検索ファクト検証不合格 ({filename}): {ext_issues}")
                    await self._insert_review(
                        filepath, filename, title, len(content),
                        final_status="rejected_external_fact",
                        blocked_reason=f"外部検索検証不合格: {', '.join(ext_issues[:3])}",
                    )
                    return {
                        "filepath": str(filepath), "title": title,
                        "status": "rejected_external_fact",
                        "mechanical": mech_result, "factual": factual_result,
                        "external_fact": ext_result, "haiku": None, "gpt5": None,
                        "cost_jpy": article_cost,
                    }
                elif search_evidence:
                    # 検証通過 + エビデンスあり → ログに記録（記事品質向上に活用）
                    logger.info(
                        f"外部検索エビデンス取得 ({filename}): {len(search_evidence)}件 — "
                        f"記事のファクト補強に活用可能"
                    )
        except Exception as e:
            logger.warning(f"外部検索ファクト検証エラー（続行、エビデンスなし）: {e}")
            # 外部検証の失敗でrejectはしない — エビデンスなしで後段に進む
            # （主張の虚偽を検出できないだけで、記事自体の品質は他の層で判定する）
        ...
        if verdict == "needs_edit" and s1_score >= 0.75:
            logger.info(
                ...
                f"(stage1_score={s1_score:.2f} >= 0.75, stage2_score={s2_score})"
            )
...
{content}"""
        ...
        try:
            ...
            # API失敗時は安全側に倒す（品質を優先）
            return {"passed": False, "critical_issues": [f"事実検証API呼び出し失敗: {e}"], "warnings": [], "fabrication_risk_score": 0.6}
    ...
    # ===== Stage 1.7: 外部検索ファクト検証 =====

    async def _external_fact_verification(self, content: str, title: str) -> dict:
        """記事の主要な事実主張を外部検索で検証する。

        「日本語記事がない/少ない」の主張を外部検索で確認し：
        - 実際に日本語記事が多数ある場合 → reject（虚偽の主張）
        - 実際に少ない場合 → pass + 検索結果をエビデンスとして返す
        - ツール名が検出された場合 → 関連情報を収集してDBに保存（記事改善・システム改善に活用）
        """
        import re
        issues = []
        search_evidence = []  # 検索で得られたエビデンス（記事生成・システム改善に活用）

        # 1. 「日本語記事がない/少ない/ゼロ」系の主張を検出
        no_jp_claims = re.findall(
            r'日本語(?:の)?(?:記事|情報|解説)(?:が)?(?:ない|なし|ゼロ|少ない|極端に少ない|ほぼ(?:ない|存在しない))',
            content
        )

        # 2. 主要な固有名詞（ツール/技術名）を抽出
        tool_mentions = re.findall(
            r'(?:話題の|注目の|海外で.*?の)「([^」]+)」',
            content
        )
        # タイトルからも抽出
        title_tools = re.findall(r'「([^」]+)」', title)
        all_tools = list(dict.fromkeys(tool_mentions + title_tools))  # 重複排除、順序維持

        # 検索が必要な場合のみ外部APIを呼ぶ
        if not no_jp_claims and not all_tools:
            return {"passed": True, "issues": [], "checked": False, "evidence": []}

        # === 外部検索実行 ===
        search_results_by_tool = {}
        try:
            from tools.tavily_client import TavilyClient
            tavily = TavilyClient()

            for tool_name in all_tools[:3]:
                ...
                jp_results = [r for r in results if any(
                    '\u3040' <= c <= '\u9fff' for c in r.get("title", "")
                )]
                all_results = results

                search_results_by_tool[tool_name] = {
                    "total_results": len(all_results),
                    "jp_results": len(jp_results),
                    "titles": [r.get("title", "")[:80] for r in all_results[:5]],
                    "urls": [r.get("url", "") for r in all_results[:5]],
                }

                # エビデンスとして保存（記事生成パイプラインで活用可能）
                for r in all_results[:3]:
                    search_evidence.append({
                        ...
                        "is_japanese": r in jp_results,
                    })

                cost = 2.0
                await self._log_llm_cost("tavily-search", "note_external_fact_check", cost)
                self.run_cost += cost

        except Exception as e:
            logger.warning(f"外部検索ファクト検証でTavily呼び出し失敗: {e}")
            try:
                from tools.jina_client import JinaClient
                jina = JinaClient()
                for tool_name in all_tools[:2]:
                    search_text = await jina.extract_markdown(
                        f"https://search.jina.ai/search?q={tool_name}+使い方+日本語"
                    )
                    has_jp_content = bool(search_text and len(search_text) > 200)
                    search_results_by_tool[tool_name] = {
                        "total_results": 1 if has_jp_content else 0,
                        "jp_results": 1 if has_jp_content else 0,
                        "source": "jina_fallback",
                    }
                    if has_jp_content:
                        search_evidence.append({
                            "tool": tool_name,
                            "title": f"{tool_name} (Jina検索結果)",
                            "snippet": search_text[:200],
                            "is_japanese": True,
                        })
                    cost = 0.5
                    await self._log_llm_cost("jina-reader", "note_external_fact_check", cost)
                    self.run_cost += cost
            except Exception as e2:
                logger.warning(f"Jinaフォールバックも失敗: {e2}")

        # === 検証結果をDBに保存（システム改善・記事改善に活用） ===
        try:
            from tools.db_pool import get_connection
            async with get_connection() as conn:
                for tool_name, sr in search_results_by_tool.items():
                    await conn.execute(
                        """INSERT INTO intel_items
                           (source, category, title, content, metadata, status, created_at)
                           VALUES ('fact_verification', 'external_search', $1, $2, $3, 'verified', NOW())
                           ON CONFLICT DO NOTHING""",
                        f"外部検索検証: {tool_name}",
                        json.dumps(sr, ensure_ascii=False),
                        json.dumps({"search_query": f"{tool_name} 使い方 解説",
                                    "evidence_count": len(search_evidence)}, ensure_ascii=False),
                    )
        except Exception as db_err:
            logger.warning(f"検索エビデンスDB保存失敗（続行）: {db_err}")

        # === 判定ロジック ===
        for tool_name, sr in search_results_by_tool.items():
            jp_count = sr.get("jp_results", 0)

            if no_jp_claims:
                if jp_count >= 3:
                    # 日本語記事が3件以上 → 「ゼロ/少ない」は虚偽
                    issues.append(
                        f"「{tool_name}」の日本語記事は実際に{jp_count}件以上存在する — "
                        f"「日本語記事がない」の主張は虚偽"
                    )
                elif jp_count == 0:
                    # 本当に日本語記事がない → 主張は正当、エビデンス付きで通過
                    logger.info(
                        f"外部検索検証: 「{tool_name}」の日本語記事は検索で0件 — 主張は妥当"
                    )
                else:
                    # 1-2件 → 「少ない」は妥当だが「ゼロ」は虚偽の可能性
                    if any("ゼロ" in p or "ない" in p for p in no_jp_claims):
                        issues.append(
                            f"「{tool_name}」の日本語記事は{jp_count}件検出 — "
                            f"「ゼロ」ではないが少ない。記述を「少ない」に修正推奨"
                        )
                        # 修正推奨だがrejectはしない（warningレベル）
                        issues.pop()  # issueから削除（warningとして別途記録）
        ...
        return {
            ...
            "search_results": search_results_by_tool,
        }
